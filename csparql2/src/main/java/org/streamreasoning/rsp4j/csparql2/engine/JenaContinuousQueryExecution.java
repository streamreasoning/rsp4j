...
public class JenaContinuousQueryExecution extends Observable implements Observer, ContinuousQueryExecution<Graph, Graph, SolutionMapping<Binding>, SolutionMapping<Binding>> {
    ...
    public JenaContinuousQueryExecution(DataStream out, ContinuousQuery query, SDS<Graph> sds, RelationToRelationOperator<SolutionMapping<Binding>, SolutionMapping<Binding>> r2r, RelationToStreamOperator<SolutionMapping<Binding>, SolutionMapping<Binding>> r2s, List<StreamToRelationOp<Graph, Graph>> s2rs) {
        this.query = query;
        this.q = (Query) query;
    ...
    public Graph apply(Binding b, long now) {
        ...
        Node s = NodeFactory.createURI(IRIs.resolve("/result/" + now));
        Node p = NodeFactory.createURI(IRIs.resolve("eventTime"));
        Node o = NodeFactory.createLiteral(now + "", XSDDatatype.XSDdateTimeStamp);
        g.add(Triple.create(s, p, o));
        p = NodeFactory.createURI(IRIs.resolve("processingTime"));
