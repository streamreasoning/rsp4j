 ...
 */
public class EPServicesContextFactoryRSP implements EPServicesContextFactory {
    ...
    public EPServicesContext createServicesContext(EPServiceProvider epServiceProvider, ConfigurationInformation configSnapshot) {
        ...
        StatementContextFactory statementContextFactory = new StatementContextFactoryRSP(plugInViews, plugInPatternObj, systemVirtualDWViewFactory);
        ...
        EPStatementFactoryRSP epStatementFactory = new EPStatementFactoryRSP();
        EPServicesContext services = new EPServicesContext(epServiceProvider.getURI(), schedulingService,
                ...
                epStatementFactory, new RegexHandlerFactoryDefault(), new ViewableActivatorFactoryDefault(),
    ...
    public enum PackageName {
        ...
        public static final EPServicesContextFactoryDefault.PackageName check(String name) {
            EPServicesContextFactoryDefault.PackageName ret = EPServicesContextFactoryDefault.PackageName.INVALID;
            int[] codePoint;
            int index = 0, dotex = -1;
                ...
                while (index <= codePoint.length) {
                    if (index == codePoint.length) {
                        if (codePoint[index - 1] == '.') {
                            ret = EPServicesContextFactoryDefault.PackageName.INVALID;
                            break escape;
                        }
                        int start = dotex + 1;
                        int end = index;
                        ...
                        if (!(ret == EPServicesContextFactoryDefault.PackageName.QUALIFIED)) ret = EPServicesContextFactoryDefault.PackageName.SIMPLE;
                        break escape;
                    }
                    if (codePoint[index] == '.') {
                        if (codePoint[index - 1] == '.') {
                            ...
                            dotex = index;
                            ret = EPServicesContextFactoryDefault.PackageName.QUALIFIED;
                        }
