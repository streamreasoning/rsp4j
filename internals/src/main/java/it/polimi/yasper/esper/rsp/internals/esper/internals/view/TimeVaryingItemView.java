...
public abstract class TimeVaryingItemView extends ViewSupport implements CloneableView, DataWindowView, ScheduleAdjustmentCallback, StoppableView, StopCallback, TimeVarying {
     ...
     */
    public TimeVaryingItemView(AgentInstanceViewFactoryChainContext agentInstanceContext, WindowOperatorFactory timeWindowViewFactory, ExprTimePeriodEvalDeltaConst timeDeltaComputation, ViewUpdatedCollection viewUpdatedCollection, TimeWindow w) {
        ...
        ScheduleHandleCallback callback = new ScheduleHandleCallback() {
            public void scheduledTrigger(EngineLevelExtensionServicesContext extensionServicesContext) {
                if (InstrumentationHelper.ENABLED) {
                    InstrumentationHelper.get().qViewScheduledEval(TimeVaryingItemView.this, TimeVaryingItemView.this.timeWindowViewFactory.getViewName());
                }
                TimeVaryingItemView.this.expire();
                if (InstrumentationHelper.ENABLED) {
    ...
    public final void update(EventBean[] newData, EventBean[] oldData) {
        ...
        if ((newData != null) && (newData.length > 0)) {
            ...
            // apply data points to the timeWindow
