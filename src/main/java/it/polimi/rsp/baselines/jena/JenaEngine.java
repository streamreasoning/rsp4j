...
import it.polimi.rsp.baselines.jena.sds.observer.DefaultTVG;
import it.polimi.rsp.baselines.jena.sds.observer.NamedTVG;
import it.polimi.rsp.baselines.jena.sds.observer.SDSObserver;
...
public abstract class JenaEngine extends RSPEsperEngine {
    ...
    public JenaEngine(BaselineStimulus eventType, EventProcessor<Response> receiver, long t0, String provider) {
        ...
        this.queries = new HashMap<Query, SDS>();
    ...
    public ContinousQueryExecution registerQuery(RSPQuery bq) {
        ...
        SDSListener listener = new SDSListener(tbox, receiver, bq, bq.getQ(), reasoning, ontology_language, "");
        ...
        if (bq.getNamedwindows() != null) {
            ...
            for (Map.Entry<Node, Window> entry : bq.getNamedwindows().entrySet()) {
                ...
                if (!listener.addNamedWindowStream(window, stream, new InfModelImpl(listener.getReasoner().bind(tbox.getGraph())))) {
        ...
        return new JenaCQueryExecution(listener, listener);
    }

    public ContinousQueryExecution registerQuery2(RSPQuery bq) {
        log.info(bq.getQ().toString());

        Model tbox = ModelFactory.createMemModelMaker().createDefaultModel();
        Model kb = ModelFactory.createMemModelMaker().createDefaultModel();

        SDSObserver sds = new SDSObserver(tbox, kb, receiver, bq, bq.getQ(), reasoning, ontology_language, "");

        int i = 0;

        //TODO Unnamed timevaryingraph receive data from more than one stmt

        DefaultTVG defTVG = new DefaultTVG(sds.getReasoningType(), Collections.EMPTY_SET);
        Model bind = sds.bind(defTVG.getGraph());
        defTVG.setGraph(bind.getGraph());
        //TODO is the default graph the same of the default stream?
        //Need to add default stream
        defTVG.addObserver(sds);
        if (bq.getWindows() != null) {
            for (Window window : bq.getWindows()) {
        ...
        if (bq.getNamedwindows() != null) {
            ...
            for (Map.Entry<Node, Window> entry : bq.getNamedwindows().entrySet()) {
                ...
                String statementName = "QUERY" + bq.getId() + "STMT_NDM" + j;
                EPStatement epl = cepAdm.create(w.toEPL(), statementName);
                NamedTVG tvg = new NamedTVG(sds.getReasoningType(), epl);

                tvg.addObserver(sds);
                bind = sds.bind(tvg.getGraph());
        ...
        //TODO create interface SDSObserver and let be two possible implementations
        queries.put(bq, sds);
        return new JenaCQueryExecution(sds, sds);
    ...
    public ContinousQueryExecution registerQuery(BaselineQuery bq) {
        ...
        SDSListener listener = new SDSListener(bq.getTbox(), receiver, bq, QueryFactory.create(bq.getSparql_query()),
        ...
        for (String[] pair : bq.getEsperNamedStreams()) {
            ...
            if (!listener.addNamedWindowStream(window, stream, new InfModelImpl(listener.getReasoner().bind(bq.getTbox().getGraph())))) {
                throw new StreamRegistrationException(
                        "Impossible to register window named  [" + window + "] on stream [" + stream + "]");
            }
        }
