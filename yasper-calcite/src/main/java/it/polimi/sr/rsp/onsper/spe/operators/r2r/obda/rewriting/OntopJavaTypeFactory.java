 ...
 */
public class OntopJavaTypeFactory extends SqlTypeFactoryImpl implements JavaTypeFactory {
    private final Map<List<Pair<Type, Boolean>>, SyntheticRecordType>
            syntheticTypes = new HashMap<>();

    public OntopJavaTypeFactory() {
        this(RelDataTypeSystem.DEFAULT);
    }

    public OntopJavaTypeFactory(RelDataTypeSystem typeSystem) {
        super(typeSystem);
    }
    ...
    public Type getJavaClass(RelDataType type) {
        ...
        switch (type.getSqlTypeName()) {
            case ROW:
                ...
                if (type instanceof OntopRecordType) {
                    return ((OntopRecordType) type).clazz;
                } else {
    ...
    public RelRecordType createStructType(DatabaseRelationDefinition dbReldef) {

        Builder b = builder();
        List<Attribute> attributes = dbReldef.getAttributes();
        for (int i = 0; i < attributes.size(); i++) {
            Attribute attribute = attributes.get(i);
            SqlTypeName typeName = attribute.getSQLTypeName().equals(Integer.class.getSimpleName()) ? SqlTypeName.INTEGER : SqlTypeName.VARCHAR;
            System.out.println(attribute.getID().getName() + " " + typeName);
            b.add(attribute.getID().getName(), typeName);
        }

        RelDataType build = b.build();


        return new OntopRecordType(build.getFieldList(), getTupleClass(dbReldef));

    }

    private Class<?> getTupleClass(DatabaseRelationDefinition table) {
