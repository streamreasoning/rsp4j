...
/**
 * This parser class is based on the RSP-QL working syntax written using ANTRL4. The parse tree visitor maps the syntax
 * to an extended Jena query class, but other query classes and language extensions can be implemented on top of this
 * visitor pattern.
 *
 * The RSPQLJenaVisitor currently has the following known limitations:
 ...
 * syntax parts of the syntax to a Jena query, while the streaming portion is placed in a RSPQLQuery.
 *
 * The SPARQLJenaVisitor currently has the following known limitations:
 * - No support for complex values in collections e.g. SELECT * WHERE { ?s ?p ( [ a <test> ] ) }
 */

public class RSPQLJenaVisitor extends RSPQLBaseVisitor {
